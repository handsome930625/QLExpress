package com.ql.util.express.instruction.env;
...
public class InstructionSet implements Serializable {
        ...
        for (int i = 0; i < instructionList.length; i++) {
            ...
            if (instruction instanceof InstructionOperator) {
                String opName = ((InstructionOperator) instruction)
                        .getOperator().getName();
                if (opName != null) {
                    // addOperator(operator)中op.name有可能为空
    ...
    public CallResult execute(RunEnvironment environment, InstructionSetContext context,
        ...
        this.executeInnerOrigInstruction(environment, errorList, log);
    ...
    private void executeInnerOrigInstruction(RunEnvironment environment, List<String> errorList, Log log) throws
            Exception {
