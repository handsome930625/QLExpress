...
public class OperatorFactory {
    ...
    public void addOperatorWithAlias(String aAliasName, String name, String errorInfo) throws Exception {
        ...
        OperatorBase origOperator = this.operator.get(name);
        if (origOperator == null) {
            throw new Exception(name + " 不能被设置别名");
        }
        OperatorBase destOperator;
        if (origOperator instanceof CanClone) {
            destOperator = ((CanClone) origOperator).cloneMe(aAliasName, errorInfo);
        } else {
            Class<OperatorBase> opClass = (Class<OperatorBase>) origOperator.getClass();
